#include "hal_gpio.h"
#include "my_nvic.h" /* Giả sử file này chứa định nghĩa NVIC */
...
bool HAL_GPIO_Init(uint32_t virtual_pin)
{
    bool retVal = true;
    PORT_Type * port = NULL;
...
bool HAL_GPIO_RegisterCallback(uint32_t virtual_pin, HAL_GPIO_Callback_t callback)
{
    bool retVal = true;
...
void HAL_GPIO_WritePin(uint32_t virtual_pin, bool value)
{
    uint32_t physical_pin = 0U;
...
bool HAL_GPIO_ReadPin(uint32_t virtual_pin)
{
    uint32_t physical_pin = 0U;
...
bool HAL_GPIO_IsInterruptFlagSet(uint32_t virtual_pin)
{
